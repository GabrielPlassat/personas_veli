...
""")
    ...
    for col, res in zip(cols5, results_t3):
            ...
            # Autonomie batterie préconisée : 80-100 km (standard industrie) vs besoin estimé
            batt_std_low  = 80 if res['vit'] >= 90 else 60 if res['vit'] >= 45 else 40
            batt_std_high = 100 if res['vit'] >= 90 else 80 if res['vit'] >= 45 else 60
            ...
            col.markdown(
                ...
                f"<div style='font-size:11px;color:#0f172a;font-weight:600;margin-bottom:2px'>🔋 Batterie : {batt_std_low}–{batt_std_high} km</div>"
                f"<div style='font-size:10px;color:#64748b;margin-bottom:4px'>(besoin estimé : {o['auto']} km réels · spec. {auto_spec} km)</div>"
    ...
    if not rural_ld_covered:
        st.markdown(
            ...
            "<b>Le problème est le prix, pas l'autonomie</b> : avec 150 km d'autonomie, 92 % de "
            "leurs trajets seraient couverts — mais le L7e 150 km coûte ~15 400 € brut, "
            "hors de portée pour 99 % de ce profil dont le budget max est ~9 000 €."
            "<br>Pour les couvrir : <b>aides ≥ 35 %</b> pour descendre sous 10 000 €, "
            "ou une baisse du coût des batteries LFP sous 20 €/km."
            "</span></div>",
            unsafe_allow_html=True
        )
