    ...
    if uploaded_json is not None:
        try:
            ...
            if "CUMUL_AUTO" in data and "AFFORD_PROFILES" in data:
                CUMUL_AUTO.update({pid:{int(k):v for k,v in d.items()} for pid,d in data["CUMUL_AUTO"].items() if pid in required})
                AFFORD_PROFILES.update({pid:{int(k):v for k,v in d.items()} for pid,d in data["AFFORD_PROFILES"].items() if pid in required})
                ...
                if "profil_pct" in data:
                    ...
                    if 0.8 <= sum(pct_map.values()) <= 1.2:
                        for _p in PROFILES_T3:
                            if _p["id"] in pct_map: _p["pct"] = float(pct_map[_p["id"]])
                st.success(f"✅ {N_CIBLE:,} personas Nemotron")
            else:
