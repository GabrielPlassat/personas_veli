...
with tab_home:
    ...
    st.divider()
    st.markdown("### 📓 Notebook Google Colab — générer des personas enrichis")
    nb_c1, nb_c2 = st.columns([3,1])
    with nb_c1:
        st.markdown("""
Le notebook `personas_mobilite_200k.ipynb` génère **200 000 personas** enrichis de données mobilité réelles :
1. Stream 2M personas Nemotron depuis HuggingFace · filtre low-income rural+périurbain
2. Attribue les stats EMP 2019 (km/jour, autonomie, vitesse min, places, budget, CO₂)
3. Exporte le CSV `personas_mobilite_200k.csv` (~160 Mo) + `distributions_nemotron.json`

Le fichier JSON peut être **uploadé dans la sidebar** pour recalibrer l'onglet 1 sur les vrais personas.
Durée : ~20 min sur Colab gratuit. Pré-requis : compte Google uniquement.
""")
    with nb_c2:
        st.markdown(
        ...
        st.markdown("**Sorties :**")
        st.markdown("- `personas_mobilite_200k.csv`\n- `distributions_nemotron.json` ← upload ici")
...
""")
    ...
    if not rural_ld_covered:
        st.markdown(
            ...
            "<b>L'écart est structurel</b> : même avec 10 % d'aides, le prix net reste hors budget."
            "<br>Pour les couvrir : <b>aides ≥ 20 %</b> (quelques centaines de personas) "
            "ou rupture sur le coût des batteries."
            "</span></div>",
            unsafe_allow_html=True
        )
