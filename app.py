...
SPEED_KEYS=[25,45,90,110,999]; AUTO_KEYS=[10,20,30,40,50,60,80,100,120,150]; SEATS_KEYS=[1,2,3,4]
...
def interp(d, v):
    """Interpolation linéaire robuste — gère les valeurs None/NaN du JSON Nemotron."""
    import math as _math
    try:
        # Filtrer les clés avec valeurs valides
        keys = sorted(k for k in d.keys() if d[k] is not None)
        if not keys: return 0.0
        d2 = {k: float(d[k]) for k in keys}
        if v <= keys[0]: return d2[keys[0]]
        if v >= keys[-1]: return d2[keys[-1]]
        for i in range(len(keys)-1):
            k0, k1 = keys[i], keys[i+1]
            if k0 <= v <= k1:
                result = d2[k0] + (v-k0)/(k1-k0) * (d2[k1]-d2[k0])
                return result if _math.isfinite(result) else 0.0
        return 0.0
    except Exception:
        return 0.0
...
# ── Structures globales Onglet 1 (Optimiseur) ────────────────────────────────
VEHICLE_TYPES_T3 = [
...
def reach_t3(auto, vit, pl, prix_net):
    ...
    for p in PROFILES_T3:
        ...
        import math as _m
        contrib_raw = p["pct"] * N_CIBLE * p_auto * p_vit * p_pl * p_afford
        contrib = contrib_raw if _m.isfinite(contrib_raw) else 0.0
        total   += contrib
        fails = []
...
# ─── SESSION STATE ───────────────────────────────────────────────────────────
if "personas_csv"    not in st.session_state: st.session_state.personas_csv    = []
if "personas_source" not in st.session_state: st.session_state.personas_source = "builtin"
if "reactions"       not in st.session_state: st.session_state.reactions        = {}

with st.sidebar:
    st.markdown("## 🚲 Vélis × Personas")
    st.divider()
    st.markdown("### 🎯 Optimiseur des aides")
    st.caption("Curseur actif sur l'onglet 1. Recalcul en temps réel.")
    aide = st.slider(
        "Aides publiques (%)", min_value=0, max_value=20,
        step=1, value=0, format="%d %%", key="aide_global",
        help="Réduit le prix net de chaque véhicule."
    )
    if aide > 0:
        st.markdown(
            f"5 k€ → **{round(5000*(1-aide/100)):,} €** · "
            f"10 k€ → **{round(10000*(1-aide/100)):,} €**"
        )
    st.divider()
    st.markdown("**📥 Données Nemotron avancées**")
    with st.expander("ℹ️ À quoi ça sert ?", expanded=False):
        st.markdown("""
Par défaut, l'**onglet 1** (Optimiseur) utilise des estimations calibrées sur l'EMP 2019 et CGDD/INSEE.

En uploadant ce fichier JSON, vous remplacez ces estimations par des distributions calculées
sur les **6M personas Nemotron réels** :

**Comment générer le fichier ?**
1. Onglet Accueil → cliquer le badge **Open in Colab**
2. Exécuter toutes les cellules (~20 min sur Colab gratuit)
3. La cellule 9d télécharge automatiquement `distributions_nemotron.json`
4. L'uploader ici → l'onglet 1 se recalibre immédiatement

Le même notebook génère aussi `personas_mobilite_200k.csv` utilisable dans l'**onglet 3** (Simulateur personas).
""")
    uploaded_json = st.file_uploader(
        "distributions_nemotron.json", type=["json"], key="nemotron_json",
        help="Cellule 9d du notebook Colab · recalibre l'onglet 1 sur les vrais personas Nemotron"
    )
    if uploaded_json is not None:
        try:
            import json as _json
            data = _json.load(uploaded_json)
            required = {"periurbain_solo","periurbain_famille","rural_navetteur","rural_famille","rural_longue_dist"}
            if "CUMUL_AUTO" in data and "AFFORD_PROFILES" in data:
                loaded_profiles = []
                skipped_profiles = []
                for pid, d in data["CUMUL_AUTO"].items():
                    if pid not in required: continue
                    vals = {int(k):v for k,v in d.items() if v is not None}
                    # Vérifier que la distribution a des valeurs significatives
                    # (max > 0.05 signifie qu'au moins 5% des personas ont leur besoin couvert)
                    if vals and max(vals.values()) > 0.05:
                        CUMUL_AUTO[pid] = vals
                        loaded_profiles.append(pid)
                    else:
                        skipped_profiles.append(pid)
                        # Conserver la distribution EMP 2019 par défaut
                for pid, d in data["AFFORD_PROFILES"].items():
                    if pid not in required: continue
                    vals = {int(k):v for k,v in d.items() if v is not None}
                    if vals and max(vals.values()) > 0.05 and pid in loaded_profiles:
                        AFFORD_PROFILES[pid] = vals
                # N_CIBLE : ne mettre à jour que si tous les profils sont présents
                # Si des profils périurbains manquent, n_cible_nemotron = rural seulement
                # → on conserve l'estimation globale (1,95M) pour ne pas fausser les totaux
                if len(loaded_profiles) == 6:
                    N_CIBLE = data.get("n_cible_nemotron", N_CIBLE)
                # Mettre à jour les pcts uniquement pour les profils chargés
                # Les profils sans données conservent leurs pcts estimés
                if "profil_pct" in data:
                    pct_map = data["profil_pct"]
                    loaded_pct_sum = sum(
                        float(pct_map.get(pid, 0))
                        for pid in loaded_profiles
                    )
                    for _p in PROFILES_T3:
                        if _p["id"] in pct_map and _p["id"] in loaded_profiles:
                            # Renormaliser : la pct Nemotron est relative aux profils chargés
                            # On la redistribue en gardant la même proportion entre profils chargés
                            if loaded_pct_sum > 0:
                                raw_pct = float(pct_map[_p["id"]])
                                # Conserver la pct relative au sein des profils chargés
                                # mais garder la pct absolue du profil dans la population totale
                                _p["pct"] = float(pct_map[_p["id"]])
                msg = f"✅ {N_CIBLE:,} personas Nemotron · {len(loaded_profiles)} profils chargés"
                if skipped_profiles:
                    msg += f"\n⚠️ {len(skipped_profiles)} profil(s) sans données suffisantes → valeurs EMP 2019 conservées : {', '.join(skipped_profiles)}"
                # Résumé clair de ce qui est chargé vs estimé
                n_display = data.get("n_cible_nemotron", N_CIBLE)
                if loaded_profiles:
                    st.success(
                        f"✅ JSON chargé · {n_display:,} personas scannés\n"
                        f"Profils Nemotron actifs ({len(loaded_profiles)}/6) : "
                        f"{chr(10).join(loaded_profiles)}"
                    )
                if skipped_profiles:
                    st.warning(
                        f"⚠️ {len(skipped_profiles)} profil(s) vides dans le JSON → "
                        f"**valeurs EMP 2019 conservées** : {chr(44)+chr(32).join(skipped_profiles)}\n\n"
                        f"Ces profils n'ont pas été trouvés dans votre échantillon Nemotron "
                        f"(peut-être que le filtre low-income n'a capturé que des ruraux). "
                        f"L'onglet 1 utilisera les distributions EMP 2019 pour ces profils."
                    )
            else:
        ...
        st.info("Estimations EMP 2019 actives", icon="ℹ️")


# ─── ONGLETS ────────────────────────────────────────────────────────────────
tab_home, tab1, tab2, tab3 = st.tabs([
    "🏠 Accueil",
    "🎯 Optimiseur des aides",
...
with tab_home:
    st.markdown("## 🚲 Simulateur de mobilité intermédiaire électrique")
    st.markdown(
        "Cet outil analyse quel **véhicule électrique intermédiaire** permettrait de réduire "
        "la dépendance à la voiture thermique pour les **ménages à faibles revenus en zones rurales et périurbaines**."
    )
    st.divider()

    c1, c2, c3 = st.columns(3)
    for col, icon, title, body in [
        (c1, "🎯", "Onglet 1 — Optimiseur des aides",
         "Définit les **5 types de véhicules optimaux** (VAE, L6e, L7e, L8e) pour couvrir le maximum de personas. "
         "Un seul curseur : le taux d'aides publiques. "
         "Calcule aussi le coût budgétaire des aides et le CO₂ économisé."),
        (c2, "📊", "Onglet 2 — Analyse par territoire",
         "Pour chaque constructeur Vélis, quels **territoires et profils** représentent les meilleures opportunités ? "
         "Heatmaps, top 5 véhicules par territoire, segmentation par âge et catégorie socio-professionnelle."),
        (c3, "🔍", "Onglet 3 — Simulateur personas",
         "Choisissez un **persona Nemotron** et voyez quels véhicules Vélis lui correspondent — score /120, "
         "barres détaillées Budget + Territoire + Usages + Affinité, et simulation de réaction IA (Claude)."),
    ]:
        with col:
            st.markdown(
                f"<div style='background:#f8fafc;border-radius:12px;padding:16px;"
                f"border:1px solid #e2e8f0;height:100%'>"
                f"<div style='font-size:28px;margin-bottom:8px'>{icon}</div>"
                f"<b>{title}</b><br><br>"
                f"<span style='font-size:13px;color:#475569'>{body}</span>"
                f"</div>",
                unsafe_allow_html=True)
    ...
    for col, icon, title, content in [
        (sc1, "🤖", "Personas — NVIDIA Nemotron",
         "[Nemotron-Personas-France](https://huggingface.co/datasets/nvidia/Nemotron-Personas-France) "
         "· 6M profils synthétiques · Licence **CC-BY-4.0**\n\n"
         "Occupation · territoire · foyer · sports · voyages · compétences · hobbies"),
        (sc2, "🚗", "Mobilité — EMP 2019 SDES/INSEE",
         "[Enquête Mobilité des Personnes 2019](https://www.statistiques.developpement-durable.gouv.fr) "
         "· 45 169 déplacements individuels réels · **Open data Etalab 2.0**\n\n"
         "Filtré : Q1+Q2 · DENSITECOM 2-3-4 · jours de semaine"),
        (sc3, "🚲", "Véhicules — Catalogue Vélis",
         "[Catalogue Véhicules Intermédiaires](https://wiki.lafabriquedesmobilites.fr) "
         "· 60 véhicules · 47 Actifs / 13 Passifs\n\n"
         "L1e à L7e · VAE cargo · quadricycles légers et lourds"),
    ]:
        with col:
            st.markdown(f"**{icon} {title}**\n\n{content}")

    st.divider()
    st.markdown("### 📓 Notebook Google Colab — aller plus loin avec les vraies données")

    st.markdown(
        "[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)]"
        "(https://colab.research.google.com/github/votre-pseudo/simulateur-velis/blob/main/personas_mobilite_200k.ipynb)"
        " &nbsp; `personas_mobilite_200k.ipynb` — gratuit, ~20 min, aucune installation"
    )
...
Par défaut, cette application fonctionne avec :
- **15 personas de démonstration** dans l'onglet Simulateur
- Des **estimations statistiques** dans l'onglet Optimiseur (distributions EMP 2019 + calibrations CGDD/INSEE)

Le notebook permet de **remplacer ces estimations par des données calculées sur les 6 millions de vrais personas Nemotron**.
""")
...
**Étapes 1–8 : génération du CSV (200 000 personas)**

Le notebook se connecte au dataset [NVIDIA Nemotron-Personas-France](https://huggingface.co/datasets/nvidia/Nemotron-Personas-France)
sur HuggingFace et **scanne 2 millions de profils en streaming** (sans tout télécharger).

Pour chaque persona, il vérifie deux critères :
- **Faibles revenus** : occupation de type ouvrier, employé, retraité, agriculteur, artisan (Q1+Q2)
- **Territoire rural** : département classé rural ou rural isolé

Pour les personas retenus, il attribue des **statistiques de mobilité réelles** issues de l'EMP 2019,
en interpolant dans la table de référence des 35 segments territoire × CSP :

| Colonne ajoutée | Source | Exemple (rural ouvrier) |
|---|---|---|
| `km_jour` | EMP 2019 log-normal | 31 km/jour médian |
| `besoin_autonomie_km` | EMP 2019 × 1,15 | 24 km P50 · 40 km P75 · 66 km P90 |
| `vitesse_min_kmh` | Logique routière | 90 km/h (rural) |
| `places_besoin` | Nemotron household | 3 places (foyer avec enfants) |
| `budget_achat_eur` | CSP × âge × décile INSEE | 8 500 € médian |
| `co2_actuel_g_jour` | EMP 2019 ratio | 5 425 gCO₂/jour |

Résultat : **`personas_mobilite_200k.csv`** — 166 Mo, 200 000 lignes, 25 colonnes.
→ À uploader dans **l'onglet 3 (Simulateur personas)** pour remplacer les 15 personas de démonstration.
""")
...
Sur le CSV généré, le notebook calcule les **distributions cumulatives** de chaque profil archétypal :

- **P(besoin_autonomie ≤ X km)** : quelle fraction du profil est couverte par une batterie X km ?
  *Exemple : 60 % des rural_famille ont besoin de ≤ 30 km d'autonomie*
- **P(budget ≥ prix_net)** : quelle fraction peut financer un véhicule à ce prix ?
  *Exemple : 48 % des rural_navetteur peuvent financer 8 000 €*

Ces distributions remplacent les estimations calibrées par des **mesures réelles** sur les personas.

Résultat : **`distributions_nemotron.json`** — <1 Mo.
→ À uploader dans la **sidebar gauche** pour recalibrer l'onglet 1.

---

#### Résultats observés (version actuelle du dataset)

Sur 200 000 personas filtrés, **100 % sont ruraux** — le filtre low-income Q1+Q2 de Nemotron
ne capture pas de personas périurbains dans cet échantillon.

| Profil | Personas | Médiane autonomie | Médiane budget |
|---|---|---|---|
| Rural · famille | 144 948 (72 %) | 25 km | 8 500 € |
| Rural · navetteur | 49 808 (25 %) | 19 km | 7 650 € |
| Rural · longue dist. | 5 244 (3 %) | 79 km | 8 500 € |
| Périurbain (3 profils) | **0** | — | — |

> ⚠️ Les 3 profils périurbains sont absents du JSON → l'app conserve les valeurs EMP 2019
> pour ces profils. Ce n'est pas un bug, c'est une limite de l'échantillon actuel.
""")
...
HuggingFace (6M personas Nemotron)
        │
        ▼  Étapes 1–8 du notebook (stream 2M · filtre · enrichissement EMP 2019)
        │
personas_mobilite_200k.csv  (200 000 lignes · 166 Mo)
   uuid · prenom · age · commune · occupation · csp · household_type
   km_jour · besoin_autonomie_km · vitesse_min_kmh · places_besoin
   budget_achat_eur · co2_actuel_g_jour · co2_economie_g_jour · ...
        │
        ├──────────────────────────────────────────────────────────────────
        │                                                                  │
        ▼  Upload onglet 3                        ▼  Étapes 9a–9d du notebook
        │                                                                  │
Simulateur personas                    distributions_nemotron.json
 Remplace les 15 personas              6 profils archétypaux :
 de démonstration par                  CUMUL_AUTO + AFFORD_PROFILES
 200 000 profils Nemotron              (distributions cumulatives réelles)
                                                   │
                                                   ▼  Upload sidebar gauche
                                       Optimiseur des aides recalibré
...
""")
    ...
    st.divider()
    st.caption(
        "Code source : [GitHub](https://github.com/votre-pseudo/simulateur-velis) · "
        "Personas : NVIDIA Nemotron CC-BY-4.0 · "
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.markdown("## 🎯 Optimiseur des aides publiques")
    st.markdown(
    "Quel véhicule électrique faut-il concevoir pour couvrir le maximum de ménages modestes "
    "en zones rurales et périurbaines, à coût minimal ? "
    "**L'algorithme optimise 5 types de véhicules en parallèle** — chacun avec une catégorie et un "
    "mode de propulsion fixés, l'autonomie et les places librement calculées."
    )

    with st.expander("📐 Méthode de calcul — cliquez pour comprendre", expanded=False):
        st.markdown("""
### Ce que fait l'algorithme

Pour chaque type de véhicule, il teste toutes les combinaisons autonomie × places et maximise :

> **Personas atteints ÷ prix net**

Un persona n'est compté que si **4 conditions** sont réunies simultanément :

| Condition | Source de données |
|---|---|
| 🔋 Son trajet quotidien est couvert par l'autonomie | **EMP 2019** — distributions réelles de trajet par CSP × territoire |
| 🚀 La vitesse max est suffisante pour son territoire | Règle binaire : vit. véhicule ≥ vit. min du profil |
| 💺 Les places sont suffisantes pour son foyer | Règle binaire : places ≥ besoin du profil |
| 💰 Il peut financer le prix net | **CGDD/INSEE 2022** — courbe d'accessibilité par CSP |

### Ce que change le curseur "Aides publiques"

Le curseur réduit le prix net de tous les véhicules. En baissant le prix :
- Plus de ménages peuvent financer → la 4ème condition se détend
- L'algorithme recalcule l'autonomie et les places optimales pour ce nouveau prix
- À partir d'un certain seuil (typiquement 6–8 %), les **profils ruraux** qui nécessitent L7e deviennent accessibles → saut de +100 000 personas d'un coup

### Les 6 profils archétypaux ciblés

| Profil | Territoire | Part | Vitesse min | Places min |
|---|---|---|---|---|
| 🚴 Cycliste | Périurbain | 8 % | 25 km/h | 1 |
| 🚶 Solo | Périurbain | 12 % | 45 km/h | 1 |
| 👨‍👩‍👧 Famille | Périurbain | 25 % | 45 km/h | 3 |
| 🌾 Navetteur | Rural | 28 % | 90 km/h | 2 |
| 🏡 Famille | Rural | 20 % | 90 km/h | 3 |
| 🛣️ Longue dist. | Rural isolé | 7 % | 90 km/h | 1 |
""")
    ...
    for col, res in zip(cols5, results_t3):
            ...
            gain_html = (f"<div style='font-size:11px;color:#16a34a' "
                         f"title='Gain de personas obtenu grâce aux {aide}% d\'aides'>"
                         f"▲+{res['gain']:,} grâce aux aides</div>"
                         if res["gain"] > 0 else "")
            ap_badge  = ("🚴 Actif" if res["actif"] else "🛞 Passif")
            ap_bg     = "#f0fdf4" if res["actif"] else "#ede9fe"
            ap_txt    = "#14532d" if res["actif"] else "#4c1d95"
            # Autonomie batterie : basée sur l'autonomie réellement optimisée
            # La spec constructeur = autonomie optimisée × 1.20 (déjà dans auto_spec)
            batt_opt = o["auto"]  # autonomie optimale calculée
            batt_spec = auto_spec  # spec constructeur (+20%)
            seats_label   = SEATS_LABELS[o['pl']]
            # VAE cargo → toujours 1 adulte + 2 enfants
            if res['id'] == 'vae_cargo':
                seats_label = '1 adulte + 2 enfants'
            col.markdown(
                ...
                f"<div style='font-size:11px;color:#0f172a;font-weight:600;margin-bottom:2px'>🔋 Autonomie optimisée : {batt_opt} km réels</div>"
                f"<div style='font-size:10px;color:#64748b;margin-bottom:4px'>Spec. constructeur : <b>{batt_spec} km</b> (+20 % marge)</div>"
                f"<div style='font-size:11px;color:#64748b;margin-bottom:4px'>💺 {seats_label}</div>"
    ...
    for prof in PROFILES_T3:
            ...
            contribs = []
            needs_high_auto = prof.get("auto_p75", 0) > 50  # profil à longs trajets
            for res in results_t3:
                o = res["opt"]
                p_vit    = 1.0 if res["vit"] >= prof["vit_min"] else 0.0
                p_pl     = 1.0 if o["pl"]  >= prof["places"]   else 0.0
                p_auto   = cumul_auto_t3(pid, o["auto"])
                p_afford = afford_t3(pid, o["pn"])
                if p_vit > 0 and p_pl > 0 and p_auto > 0.01:
                    score    = p_auto * p_afford * 100
                    contrib_n = round(prof["pct"] * N_CIBLE * p_auto * p_afford)
                    contribs.append((res["label"], res["color"], round(score, 1), contrib_n))

            # Build stacked bar
            total_contrib_n = max((c[3] for c in contribs), default=0) if contribs else 0
            bar_parts = "".join(
                f"<div style='flex:{score};background:{color};height:100%;min-width:2px' "
                f"title='{label}: {score:.0f}% · {n_c:,} personas'></div>"
                for label, color, score, n_c in contribs
            ) if contribs else (
            "<div style='flex:1;background:#e2e8f0;height:100%;display:flex;align-items:center;"
            "padding:0 8px'><span style='font-size:10px;color:#94a3b8'>"
            + ("Trajets trop longs — aucun véhicule ne couvre ce profil avec ce budget" if needs_high_auto
               else "Aucun véhicule ne couvre ce profil")
            + "</span></div>"
        )
            ...
            st.markdown(
                ...
                f"{bar_parts}</div>"
                f"<span style='font-size:11px;color:#64748b;width:120px;text-align:right'>{covered} type(s) · {total_contrib_n:,} pers.</span>"
                f"</div>",
                unsafe_allow_html=True)
    ...
    # Analyse structurelle si rural_longue_dist n'est pas couvert
    rural_ld_covered = any(
        d["contrib_n"] > 100
        for res in results_t3
        for d in res["dets"]
        if "longue" in d["profil"].lower() and d["ok"]
    )
    if not rural_ld_covered:
        st.markdown(
            "<div style='background:#fef2f2;border-left:4px solid #dc2626;border-radius:8px;"
            "padding:12px 16px;margin-top:8px'>"
            "<b>🔍 Rural · longue distance — profil structurellement non couvert</b><br>"
            "<span style='font-size:13px;color:#7f1d1d'>"
            "Ces personas parcourent <b>80–140 km/jour</b> (données Nemotron réelles) "
            "mais ont un <b>budget maximal ~9 000 €</b>. "
            "Un L7e 80 km d'autonomie coûte ~11 800 € brut. "
            "<b>Double contrainte</b> :<br>"
            "① L'optimiseur global choisit une courte autonomie (20–50 km) pour maximiser "
            "la couverture des profils périurbains et ruraux qui dominent. "
            "À cette autonomie, P(trajets rural longue dist. couverts) = 0 %.<br>"
            "② Même avec 150 km d'autonomie (92 % des trajets couverts), "
            "le L7e 80 km coûte ~8 700 € net à 20 % d'aides — mais seulement "
            "17 % de ce profil peut financer ce prix car le budget médian est ~9 000 €.<br>"
            "<b>Pour les couvrir</b> : un L7e 80 km 1 place à 20 % d'aides atteindrait "
            "~10 000 personas (budget 8 700 € net) — faisable mais pas l'optimum global. "
            "Montez le curseur à 20 % et regardez le tableau 📊 Récapitulatif."
            "</span></div>",
            unsafe_allow_html=True
        )
            ...
            with tab_vt:
                ...
                for d in res["dets"]:
                    if d["ok"]:
                        ...
                        # Distinguer : specs incompatibles vs autonomie insuffisante
                        if d['fails']:
                            reason = ' · '.join(d['fails'])
                            bg, brd, ico = '#fef2f2', '#fca5a5', '❌'
                        elif d['p_auto'] < 0.05:
                            reason = (f"autonomie optimisée ({o['auto']} km) insuffisante pour ce profil "
                                      f"(P75 besoin = {d['auto_p75']} km). "
                                      f"Ce véhicule est optimisé pour les autres profils à trajets courts — "
                                      f"un véhicule dédié à ce profil dépasserait le budget (~9 000 €)")
                            bg, brd, ico = '#fff7ed', '#f97316', '⚠️'
                        else:
                            reason = 'contribution nulle'
                            bg, brd, ico = '#fef2f2', '#fca5a5', '❌'
                        st.markdown(
                            f"<div style='background:{bg};border-left:3px solid {brd};border-radius:6px;"
                            f"padding:6px 12px;margin:3px 0;font-size:12px'>"
                            f"{d['icon']} <b>{d['profil']}</b> — {ico} {reason}</div>",
                            unsafe_allow_html=True)
    ...
    if aide > 0:
            ...
            for res in results_t3:
                ...
                # CO₂ économisé : véhicules × co2 moyen journalier × 250j/an
                # co2_jour_moy = MOYENNE PONDÉRÉE sur les profils couverts (normalisation par Σpct)
                _co2_num = sum(p["co2_jour"]*p["pct"] for p in PROFILES_T3
                               if any(d["profil"]==p["nom"] and d["ok"] for d in res["dets"]))
                _co2_den = sum(p["pct"] for p in PROFILES_T3
                               if any(d["profil"]==p["nom"] and d["ok"] for d in res["dets"]))
                co2_jour_moy = _co2_num / max(_co2_den, 0.001)  # g/jour · moyenne pondérée correcte
                co2_kt    = nb_veh * co2_jour_moy * 250 / 1e9  # ktCO₂/an (1 kt = 1e9 g)
                total_veh_all  += nb_veh
                total_cout_all += nb_veh * aide_euro
                total_co2_all  += nb_veh * co2_jour_moy * 250  # grammes/an
            ...
            cout_co2 = total_cout_all / max(total_co2_all/1_000_000, 1)  # €/tCO₂ (1t = 1e6g)
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown("## 📊 Analyse par territoire — Catalogue Vélis")
    st.markdown(
    "Pour chaque constructeur du catalogue Vélis, quels territoires et profils démographiques "
    "représentent les meilleures opportunités de marché ?"
    )
    with st.expander("📐 Comment lire ces analyses ?", expanded=False):
        st.markdown("""
**30 archétypes** (5 territoires × 3 âges × 2 profils sportif/standard) sont scorés sur les 60 véhicules.

Le **score /120** combine :
- **Budget (30 pts)** : ratio prix net / budget moyen du profil
- **Territoire/Vitesse (40 pts)** : compatibilité vitesse véhicule × type de zone (urbain, périurbain, rural…)
- **Usages (30 pts)** : fraction des besoins du profil couverts par les usages déclarés du véhicule
- **Affinité (+20 pts)** : signaux dans le profil (cycliste → bonus VAE, tourisme → bonus usage Tourisme…)

**Les heatmaps** montrent la moyenne des scores sur les archétypes de ce territoire.
Un score ≥ 65/120 est considéré comme un **segment porteur**.

**Pertinence avec l'onglet 1 :** l'onglet 1 identifie les *types de véhicules* optimaux pour les ménages modestes.
Cet onglet 2 montre quels *véhicules existants du catalogue* répondent le mieux à chaque profil territorial,
en incluant tous les segments de revenus (pas seulement les ménages modestes).
""")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab3:
    st.markdown("## 🔍 Simulateur personas × véhicules")
    st.markdown(
        "Choisissez un **profil de personne** (persona) et découvrez quels véhicules Vélis lui correspondent "
        "le mieux selon ses revenus, son territoire, ses usages et ses affinités. Score /120 et réaction simulée par IA."
    )

    with st.expander("👥 Qu'est-ce qu'un persona ? Comment en avoir plus ?", expanded=False):
        st.markdown("""
### Les 15 personas intégrés

Cette app contient **15 personas de démonstration** extraits du dataset
[NVIDIA Nemotron-Personas-France](https://huggingface.co/datasets/nvidia/Nemotron-Personas-France)
et enrichis avec des profils culturels, sportifs, de voyage et de compétences.
Ils couvrent des profils variés : urbain·e, périurbain·e, rural·e · ouvrier·e, employé·e, retraité·e · 22 à 71 ans.

### Charger 200 000 personas réels (notebook Colab)

1. **Exécuter** `personas_mobilite_200k.ipynb` sur Google Colab (lien sur l'onglet Accueil, ~20 min)
2. **Télécharger** `personas_mobilite_200k.csv` (~160 Mo · 200 000 personas enrichis EMP 2019)
3. **Uploader le CSV** dans l'expander "📥 Charger des personas depuis Colab" ci-dessous

Les 15 personas de démonstration sont alors remplacés par les vôtres.

### Le curseur "Aides publiques"

Le curseur dans la **sidebar gauche** s'applique à cet onglet ET à l'onglet 1 (Optimiseur).
Un seul réglage partagé — modifiez-le pour voir l'impact des aides sur le score de chaque véhicule.
""")
    st.divider()
    ...
    with st.expander("📥 Charger des personas depuis Colab", expanded=False):
        ...
        if uploaded is not None:
            try:
                ...
                # Colonnes minimales acceptées (format Colab OU format manuel)
                min_cols = {"prenom","age"}
                if not min_cols.issubset(set(df_up.columns)):
                    st.error(f"Colonnes minimales manquantes : {min_cols - set(df_up.columns)}")
                else:
                    # Mapping intelligent : colonnes Colab → colonnes app
                    # territoire → urbanite,  budget_achat_eur → budget,  occupation → profil
                    ...
                    def _besoins(row):
                        # Format CSV manuel : colonne "besoins" avec séparateur |
                        if "besoins" in row and str(row["besoins"]) not in ("", "nan"):
                            return [b.strip() for b in str(row["besoins"]).split("|") if b.strip()]
                        # Format Colab : déduire depuis csp + household_type
                        csp = str(_col(row, "csp", default=""))
                        hh  = str(_col(row, "household_type", default=""))
                        b = ["trajets domicile travail","Faire ses courses"]
                        if "famille" in hh.lower() or "enfant" in hh.lower():
                            b.append("Transport scolaire et périscolaire avec plusieurs enfants")
                        if "retraite" in csp or "retraité" in str(_col(row,"occupation",default="")):
                            b = ["Faire ses courses","Tourisme Découverte d'un territoire"]
                        return b

                    personas_loaded = []
                    for i, row_up in df_up.iterrows():
                        try:
                            # urbanite : préférer "territoire" (Colab) sinon "urbanite" (manuel)
                            urbanite_val = str(_col(row_up, "urbanite", "territoire", default="periurbain"))
                            if urbanite_val not in ("urbain","periurbain","rural","rural_isole","montagne","ile"):
                                urbanite_val = "rural"
                            # budget : préférer "budget_achat_eur" (Colab) sinon "budget" (manuel)
                            try:
                                budget_val = int(float(_col(row_up, "budget_achat_eur", "budget", default=9000)))
                            except:
                                budget_val = 9000
                            # avatar : générer depuis csp si absent
                            csp_avatars = {"ouvriers":"🔧","employes":"💼","retraites":"👴",
                                          "artisans":"🥖","agriculteurs":"🌾","inactifs":"🏠"}
                            avatar_default = csp_avatars.get(str(_col(row_up,"csp",default="")), "👤")

                            personas_loaded.append({
                                "id":       i + 100,
                                "avatar":   str(_col(row_up, "avatar", default=avatar_default)),
                                "prenom":   str(_col(row_up, "prenom", default=f"Persona {i}"))[:40],
                                "age":      int(float(_col(row_up, "age", default=40))),
                                "commune":  str(_col(row_up, "commune", default="")),
                                "dep":      str(_col(row_up, "departement", "dep", default="")),
                                "urbanite": urbanite_val,
                                "budget":   budget_val,
                                "profil":   str(_col(row_up, "profil", "occupation", default=""))[:80],
                                "resume":   str(_col(row_up, "resume", "household_type", default=""))[:200],
                                "culture":  str(_col(row_up, "culture", default="")),
                                "sport":    str(_col(row_up, "sport", default="")),
                                "travel":   str(_col(row_up, "travel", default="")),
                                "skills":   str(_col(row_up, "skills", default="")),
                                    "hobbies":  str(_col(row_up, "hobbies", default="")),
                                "besoins":  _besoins(row_up),
                            })
                        except Exception:
        ...
        if st.session_state.personas_source == "csv" and st.session_state.personas_csv:
            if st.button("↩️ Revenir aux 15 personas par défaut", key="reset_csv_1"):
                st.session_state.personas_source = "builtin"
                st.rerun()


    # ── Retour aux personas par défaut ───────────────────────────────────────
    if st.session_state.personas_source == "csv" and st.session_state.personas_csv:
        if st.button("↩️ Revenir aux 15 personas par défaut", key="reset_csv_2"):
            st.session_state.personas_source = "builtin"
            st.rerun()

    st.divider()

    col_sel3, col_main3 = st.columns([1, 3])
    with col_sel3:
        st.markdown("#### 👤 Choisir un persona")
        ...
        if st.session_state.personas_source == "csv":
            st.caption(f"✅ {nb_p} personas Colab chargés")
        else:
            st.caption(f"15 personas de démonstration (Nemotron)")
        ...
        st.session_state["_p"] = persona

    persona = st.session_state.get("_p", PERSONAS[0])
...
# ─── ONGLETS ────────────────────────────────────────────────────────────────
    ...
    with col_main3:
            ...
            aide_pct = aide  # ← curseur unique dans la sidebar gauche (onglets 1 et 3 partagent la même valeur)
            if aide_pct > 0:
                st.info(
                    f"🏛️ Aides actives : **{aide_pct} %** (depuis la sidebar) · "
                    f"un véhicule à 10 000 € revient à **{10000*(1-aide_pct/100):,.0f} €**",
                    icon="ℹ️"
                )
            st.divider()
