...
with tab3:
    # Curseur aides dans la sidebar principale
    with st.sidebar:
        st.divider()
        st.markdown("### 🎯 Optimiseur — Aides publiques")
        st.caption("Visible sur toute la page, s'applique à tous les indicateurs de l'onglet 3.")
        aide3 = st.slider(
            "Aides publiques (%)",
            min_value=0, max_value=20, step=1, value=0, format="%d %%",
            key="aide_optimiseur",
            help="Réduit le prix net. Chaque point débloque de nouveaux segments ruraux."
        )
        st.markdown(f"""
- 5 000 € → **{round(5000*(1-aide3/100)):,} €**
- 8 000 € → **{round(8000*(1-aide3/100)):,} €**  
- 12 000 € → **{round(12000*(1-aide3/100)):,} €**
""")
        st.divider()
        st.caption("⚠️ Modèle basé sur 5 profils archétypaux EMP 2019. Voir notebook `personas_mobilite_500k.ipynb` pour les vraies distributions Nemotron.")

    # Renommer aide → aide3 dans le code de l'optimiseur
    aide = aide3  # alias local pour réutiliser tout le code ci-dessous sans le réécrire

    st.markdown("## 🎯 Optimiseur — Véhicule intermédiaire pour les territoires ruraux")

    with st.expander("📖 Comment ce modèle est-il construit ? — Cliquez pour comprendre les données utilisées", expanded=False):
        st.markdown("""
### Ce que fait ce modèle

Ce modèle fait du **reverse engineering** : à partir des besoins réels de mobilité de la population cible,
il calcule les caractéristiques du véhicule (autonomie, vitesse, places, prix) qui maximise le nombre de personas couverts.

---

### Ce qui est réel — données EMP 2019

Les **distributions d'autonomie** par profil proviennent des micro-données de l'EMP 2019 (SDES/INSEE) :
- 45 169 déplacements individuels réels · Q1+Q2 · DENSITECOM 2-3-4
- Besoin autonomie = trajet max × 2 (AR) × 1,15 (marge batterie)

Les **courbes d'accessibilité budgétaire** sont calibrées sur CGDD/INSEE 2022 par CSP dominante.

---

### ⚠️ Ce qui est estimé — pas encore des personas Nemotron

La population cible (1,95M) et les parts des 5 profils sont des **estimations**, pas un comptage réel sur le dataset Nemotron.

**Pour passer aux vraies distributions :** exécuter le notebook `personas_mobilite_500k.ipynb` qui streame les 6M personas
depuis [HuggingFace](https://huggingface.co/datasets/nvidia/Nemotron-Personas-France), attribue les stats EMP 2019 réelles
à chaque persona, et exporte les distributions recalibrées.
""")

    st.caption(f"Population cible estimée : **{N_CIBLE:,} personas** · faibles revenus Q1+Q2 · rural + périurbain · Distributions : EMP 2019 réelles · Budget : CGDD/INSEE estimé")
    st.divider()

    opt_t3 = optimize(aide)
    r_tot_t3, dets_t3 = reach(opt_t3["auto"], opt_t3["vit"], opt_t3["pl"], opt_t3["pn"])
    opt0_t3 = optimize(0)
    r0_t3, dets0_t3 = reach(opt0_t3["auto"], opt0_t3["vit"], opt0_t3["pl"], opt0_t3["pn"])
    gain_t3 = r_tot_t3 - r0_t3
    vehicle_switched_t3 = (opt_t3["vit"]!=opt0_t3["vit"]) or (opt_t3["auto"]!=opt0_t3["auto"]) or (opt_t3["pl"]!=opt0_t3["pl"])
    dets0_by_profil_t3  = {d["profil"]:d for d in dets0_t3}
    color_r_t3 = "#16a34a" if r_tot_t3/N_CIBLE>=0.25 else "#2563eb" if r_tot_t3/N_CIBLE>=0.15 else "#d97706"

    # ── Réutilise toutes les fonctions de l'optimiseur avec les variables renommées ──
    opt=opt_t3; opt0=opt0_t3; r_tot=r_tot_t3; dets=dets_t3; r0=r0_t3; dets0=dets0_t3; gain=gain_t3
    vehicle_switched=vehicle_switched_t3; dets0_by_profil=dets0_by_profil_t3; color_r=color_r_t3

    # ── SPÉCIFICATIONS OPTIMALES ──────────────────────────────────────────────────
    st.markdown("### 🏆 Spécifications optimales calculées")
    st.markdown("""
    > **Comment ce résultat est calculé ?**  
    > L'algorithme teste toutes les combinaisons possibles (autonomie × catégorie × places) et cherche celle qui maximise
    > le ratio **personas atteints ÷ prix net**. La contrainte est simple : chaque persona n'est compté que si  
    > (1) son trajet quotidien réel est couvert par l'autonomie, (2) la vitesse autorisée suffit pour son territoire,  
    > (3) les places sont suffisantes pour son foyer, et (4) il peut financièrement accéder au prix net.  
    > Bouger le curseur d'aides recalcule tout en temps réel.
    """)
    cols=st.columns(5)
    ...
    > Chaque profil représente une fraction de la population cible (ex. "Rural · navetteur" = 28 % des 1,95M personas).  
    > **Barre gauche — couverture autonomie** : % du profil dont le trajet réel (EMP 2019) est couvert par l'autonomie du véhicule optimal.  
    > *Ex. 60 % signifie que 6 ouvriers ruraux sur 10 ont un trajet quotidien ≤ à l'autonomie proposée.*  
    > **Barre droite — accessibilité budget** : % du profil pouvant financer le prix net, selon les revenus réels de ce groupe CSP (CGDD 2022).  
    > Les flèches ▲▼ indiquent l'évolution par rapport à 0 % d'aides.
    """)
    ...
    for ap in range(0,21,2):
        ...
        if prev_opt is not None:
            ...
            # Note: delta personas and prix are always positive/negative (noise) — not shown here.
            # Only structural changes (specs, profiles) are meaningful.
        ...
        delta_pers = f"+{r2-prev_opt['r']:,}" if prev_opt and r2>prev_opt['r'] else ("—" if not prev_opt else f"{r2-prev_opt['r']:,}")
        rows.append({
            "Aide":              f"{ap} %",
            ...
            "Δ personas":        delta_pers,
            "Changements de specs": " · ".join(evol_parts) if evol_parts else "— (référence)" if ap==0 else "—",
        })
        prev_opt={**o2,"r":r2}
