        ...
        if uploaded_json is not None:
            try:
                ...
                if "CUMUL_AUTO" in data and "AFFORD_PROFILES" in data:
                    ...
                    n_cible_real = data.get("n_cible_nemotron", N_CIBLE)
                    N_CIBLE = n_cible_real  # mise à jour de la population cible
                    # Mettre à jour les parts des profils si présentes dans le JSON
                    if "profil_pct" in data:
                        pct_map = data["profil_pct"]
                        pct_sum = sum(pct_map.values())
                        if 0.8 <= pct_sum <= 1.2:  # Sanity check : doit sommer à ~1
                            for p in PROFILES_T3:
                                if p["id"] in pct_map:
                                    p["pct"] = float(pct_map[p["id"]])
                    st.success(f"✅ Nemotron chargé · {N_CIBLE:,} personas cibles réels")
                else:
