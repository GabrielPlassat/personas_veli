...
# ─── ONGLETS ─────────────────────────────────────────────────────────────────
tab1, tab2 = st.tabs(["🔍 Simulateur personas", "📊 Analyse constructeurs × territoires"])
            ...
            with st.container(border=True):
                ...
                with c2:
                    ...
                    st.markdown(
                        ...
                        f"<span style='background:{prix_bg2};color:{prix_color};padding:2px 8px;border-radius:4px;font-size:12px;font-weight:600'>{prix_str}</span>"
                        ...
                        if st.button(f"✦ Simuler la réaction de {persona['prenom'].split()[0]}", key=f"btn_{key}"):
                            with st.spinner("Génération en cours…"):
                                ...
                                prompt = (
                                    ...
                                    f"Concret sur le territoire, le prix, l'usage quotidien."
                                )
                                try:
...
with tab2:
    ...
    st.dataframe(
        pivot.style
            .background_gradient(cmap="RdYlGn", vmin=20, vmax=85, axis=None)
            .format("{:.0f}"),
