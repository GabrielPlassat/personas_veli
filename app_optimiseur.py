...
for d in dets:
    if d["ok"]:
        ...
        st.markdown(
            ...
            f"<div><div style='font-size:11px;color:#64748b;margin-bottom:2px'>💰 Accessibilité budget — revenus propres à ce profil (CSP {d['profil'].split('·')[1].strip() if '·' in d['profil'] else d['profil']})</div>"
...
for ap in range(0,21,2):
    ...
    rows.append({
        "Aide":       f"{ap} %",
        "Autonomie":  f"{o2['auto']} km  →  spec. {round(o2['auto']*1.2)} km (+20 % marge)",
        "Catégorie":  SPEED_LABELS[o2["vit"]].split("  ")[0].strip(),
        "Places":     SEATS_LABELS[o2["pl"]],
        "Prix net":   f"{o2['pn']:,} €",
        "Personas":   f"{r2:,}",
        "% cibles":   f"{r2/N_CIBLE*100:.1f} %",
        "Évolution vs niveau précédent": " · ".join(evol_parts) if evol_parts else "— (point de départ)" if ap==0 else "— (stabilité)",
    })
    prev_opt={**o2,"r":r2}
